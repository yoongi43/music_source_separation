...
def get_datasets2(args):
    train_set, valid_set = get_musdb_wav_datasets(args)
    if args.wav:
        extra_train_set, extra_valid_set = get_wav_datasets(args)
    ...
    if args.wav2:
        extra_train_set, extra_valid_set = get_wav_datasets(args, "wav2")
        weight = args.wav2_weight
        if weight is not None:
            b = len(train_set)
            e = len(extra_train_set)
            reps = max(1, round(e / b * (1 / weight - 1)))
    ...
    if args.valid_samples is not None:
        valid_set = random_subset(valid_set, args.valid_samples)
    return train_set, valid_set
